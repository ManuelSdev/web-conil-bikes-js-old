...
import BookingInfoHandler from './BookingInfoHandler'
...
export default async function CalendarPage({ params, searchParams }) {
   ...
   return (
      <div className="flex">
         ...
         <div className={cn({ 'flex-none': true, 'hidden xl:block': date })}>
            <Card {...cardProps}>
         ...
         <div
            className={cn({
               'flex-initial': true,
               'hidden xl:block': bookingId,
            })}
         >
            <Suspense fallback={<p>Cargando...</p>}>
               <BookingListHandler urlDate={encodedDate} date={date} />{' '}
            </Suspense>
         </div>
         <Suspense fallback={<p>Cargando...</p>}>
            <BookingInfoHandler bookingId={bookingId} />
         </Suspense>{' '}
      </div>
   )
}
